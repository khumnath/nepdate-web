...
export const BlogCard: React.FC<BlogCardProps> = ({ blog, onClick, compact = false }) => {
            ...
            <div className={`relative overflow-hidden ${compact ? 'h-32' : 'h-48'} bg-gray-200 dark:bg-gray-700`}>
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                    {blog.tags.slice(0, 1).map((tag, i) => (
                        !['9', '10', '11'].includes(tag) && ( // Don't show numeric month tags
                            <span key={i} className="text-[10px] uppercase font-bold text-white bg-blue-600/90 px-2 py-1 rounded-md backdrop-blur-sm">
                                {tag}
                            </span>
                        )
                    ))}
